...
def account():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Your acoount has been updated!', 'success')
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash('Your post ahas been created!', 'success')
        return redirect(url_for('home'))
...
If you did not make this requeset then simply ignore this email and no change will be made.
'''
    mail.send(msg)
